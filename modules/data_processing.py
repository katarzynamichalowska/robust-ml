...
def preprocess_data(data, var_Y, vars_X, t_len=20, scaling_method="standard", test_size=0.2, random_state=42):
    """
    Preprocesses the dataset: selects variables, reshapes into subseries, splits into train/test, and scales data.

    Args:
        data (pd.DataFrame): Input dataset.
        target (str): Target variable name.
        vars_chosen (list): List of selected feature names.
        t_len (int): Length of time series subseries.
        scaling_method (str): Scaling method ('standard', 'minmax', 'robust', etc.).
    ...
    data = data[vars_X + var_Y]

    # Reshape data into subseries
    data = reshape_into_subseries(data, t_len=t_len)
    ...
    # Scale data
    X_train_scaled, X_scaler = scaling(X_train, scaling_method)
    y_train_scaled, y_scaler = scaling(y_train, scaling_method)
...
# Example usage:
# processed_data = preprocess_data(data, target, vars_chosen, t_len=20)
# X_train, y_train = processed_data["X_train_scaled"], processed_data["y_train_scaled"]
